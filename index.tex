...
First of, our bindings have up to 40\% less overhead, as the original
libsvm Python bindings: they are implemented with Cython's numpy support
to avoid memory copies. Second, we've patched libsvm to work efficiency
...
version also provides unique features, such as working natively with
...
routines for fast array multiplication. It achieves the same order of
...
\paragraph{Objects specified by interface, not by inheritance}
%
...
on a specified parameter grid, maximizing a score function that default
...
estimators by exploiting there specific properties, such as warm restarts
...
fit. {\tt Pipeline} behaves as a standard estimator, and can be thus used
...
learning, providing an unequaled framework for easy comparison of methods
on a given application.
%
As it relies on the scientific Python ecosystem, it can be easily
integrated in a variety of applications, outside the tradition range of
statistical data analysis. Importantly, the algorithms can be used as
building blocks for efficient approaches dedicated to specific use cases,
for example in medical imaging \citep{Michel2011}.
