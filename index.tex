...
extensible, as much as possible is coded in the Python
language. However, because of their high quality and compatible
license, we also incorporate the C/C++ libraries LibSVM and LibLinear
and provide high-quality, low-overhead bindings for these libraries.

\paragraph{Simple data format exchange}

Use simple data structures for IO, Numpy arrays or
scipy.sparse. Excellent communication with other data formats, python
packages and also with C/Fortran libraries.

\paragraph{Code by interface, not by inheritance}

Inheritance is not enforced at any stage. In its simplest form, an
estimator is simply an object that implements a `fit` and `predict`
