...
general-purpose language, it is increasingly employed both in academic
...
\citep{hanke2009} which relies heavily on R, and \emph{iv)} it focuses on imperative
...
%
In order to lower the barrier of entry, we avoid extraneous framework code and keep the number
of different objects to a minimum.
The package is built
around the numpy array \citep{Vanderwalt2011}, which is commonly used throughout the
...
\noindent{\bf Numpy}:
%
the base data structure used to represent data and model parameters.
...
inheritance is not enforced at any stage; instead, code
conventions are used to provide a consistent interface. 
The central object is an {\tt estimator}, that implements a
{\tt fit} method, accepting as arguments an input data array and,
optionally, a target array of labels for supervised problems. Supervised estimators,
...
goodness of fit.  Depending on the estimator the score may correspond
to a log-likelihood, or a
negated loss function, with the guiding principle that it must always
increase with the goodness of fit.
%
The only other important object is the \emph{cross-validation iterator}:
an iterable object which provides a pair of train and test indices to split input
data accordingly. It may be a standard list, or a Python generator
which dynamically
creates these indices. Examples comprise K-fold, leave one out, or
stratified cross-validation schemes. 
...
combined estimator to \emph{e.g.} apply dimensionality reduction before
...
\smallskip \noindent{\bf SVM}:
%
While all of the packages being compared call libsvm in the
background, the 
...
\citep{LARS}. {\sl Pymvpa} uses this implementation directly via the Rpy R
...
%
Future work includes the addition of \emph{online} learning, to scale to
large datasets.
